...
with st.sidebar:
    ...
    st.markdown("<h1 style='text-align: center;'>Crop Recommendation System using Random Forest Algorithm</h1>",
...
st.markdown(
    "<h3 style='text-align: center;'>Please input the feature values to predict the best crop to plant.</h3><br>",
    unsafe_allow_html=True)
...
with col3:
    n_input = st.number_input('Insert N (kg/ha) value:', min_value=0, max_value=140,
                              help='Insert here the Nitrogen density (kg/ha) from 0 to 140.')
    p_input = st.number_input('Insert P (kg/ha) value:', min_value=5, max_value=145,
                              help='Insert here the Phosphorus density (kg/ha) from 5 to 145.')
    k_input = st.number_input('Insert K (kg/ha) value:', min_value=5, max_value=205,
                              help='Insert here the Potassium density (kg/ha) from 5 to 205.')
    temp_input = st.number_input('Insert Avg Temperature (ºC) value:', min_value=9., max_value=43., step=1.,
                                 format="%.2f", help='Insert here the Avg Temperature (ºC) from 9 to 43.')

with col5:
    hum_input = st.number_input('Insert Avg Humidity (%) value:', min_value=15., max_value=99., step=1., format="%.2f",
                                help='Insert here the Avg Humidity (%) from 15 to 99.')
    ph_input = st.number_input('Insert pH value:', min_value=3.6, max_value=9.9, step=0.1, format="%.2f",
                               help='Insert here the pH from 3.6 to 9.9')
    rain_input = st.number_input('Insert Avg Rainfall (mm) value:', min_value=21.0, max_value=298.0, step=0.1,
                                 format="%.2f", help='Insert here the Avg Rainfall (mm) from 21 to 298')
...
if predict_btn:
    ...
    with colb:
        ...
        with col1:
            st.markdown(
                f"<br><h5 style='text-align: right;'>Best Crop to Plant </h5> <h3 style='text-align: right; "
                f"line-height: 0'> <b>{rdf_predicted_value[0]} </b></h3>",
                unsafe_allow_html=True)
        ...
        st.markdown(
            f"""<h5 style='text-align: center;'>Statistics Summary about NPK and Weather Conditions values for <b> {rdf_predicted_value[0]}
